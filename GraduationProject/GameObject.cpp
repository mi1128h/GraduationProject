...
CSkyBox::CSkyBox(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature) : CGameObject(6)
{
	...
	pSkyBoxShader->CreateShaderResourceViews(pd3dDevice, pSkyBoxTexture, 0, 5);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CBillAnimObject::CBillAnimObject(int nMeshes) : CGameObject(nMeshes)
{
}

CBillAnimObject::~CBillAnimObject()
{
}

void CBillAnimObject::Animate(float fElapsedTime, CCamera* pCamera)
{
	m_fFrame += fElapsedTime * 10.0f;
	m_nTextureIndex = ((int)m_fFrame) % m_nTexCount + MinIndex;
	...
	switch (ANIM_STATE)
	{
		case ANIM_LOOP:
			break;
		case ANIM_ONE:
			int Index = m_nTexCount+MinIndex-1;
			if (m_nTextureIndex == Index) SetActive(false);
			break;
	}
...
void CBillAnimObject::SetLookAt(XMFLOAT3& xmf3Target)
{
...
void CBillAnimObject::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	CGameObject::UpdateShaderVariables(pd3dCommandList);
	pd3dCommandList->SetGraphicsRoot32BitConstants(8, 1, &m_nTextureIndex, 0);
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
CBulletObject::CBulletObject(float fEffectiveRange, int nMeshes): CBillAnimObject(nMeshes)
{
...
void CBulletObject::ChangeBullet(int ChangeIndex)
{
	switch (ChangeIndex)
	{
	case 0:
...
bool CBulletObject::CheckFire()
{
	if (!m_bActive) return false;
	if (m_nMeshIndex != 0) return false;
	return true;
}
...
void CBulletObject::Animate(float fElapsedTime, CCamera* pCamera)
{
	...
	switch (m_nMeshIndex)
	{
		...
		break;
		case 1:
			CBillAnimObject::Animate(fElapsedTime, pCamera);
		break;
	}
...
void CBulletObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	if (m_ppMeshes)
	{
		if (m_ppMeshes[m_nMeshIndex]) m_ppMeshes[m_nMeshIndex]->Render(pd3dCommandList);
	}
}
...
CParticleObject::CParticleObject(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, XMFLOAT3 xmf3Position, XMFLOAT3 xmf3Velocity, XMFLOAT3 xmf3Acceleration, XMFLOAT3 xmf3Color, XMFLOAT2 xmf2Size, float fLifetime, UINT nMaxParticles) : CGameObject()
{
	...
	pParticleTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Assets/Image/flare0.dds", RESOURCE_TEXTURE2D, 0);
	...
	pShader->CreateCbvSrvUavDescriptorHeaps(pd3dDevice, 1,2,0);
...
void CParticleObject::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	...
	if (m_pMaterial)
	{
		...
		if (m_pMaterial->m_pTexture) m_pMaterial->m_pTexture->UpdateGraphicsShaderVariables(pd3dCommandList);
