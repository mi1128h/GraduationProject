...
VS_PARTICLE_INPUT VSParticleDraw(VS_PARTICLE_INPUT input)
{
	...
	if (output.size.x <= size_x * 4.0f)
	{
		float2 size = float2(size_x * 3.5f, size_y * 3.5f) * 10.5f ;
		output.size += size * gfElapsedTime;
	}
	output.position += (0.5f * input.acceleration * input.velocity * gfElapsedTime * 10.0f);
